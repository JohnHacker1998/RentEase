...
RE.landlordPages.auth = {
  async login(app) {
    app.innerHTML = `
      <div class="auth-card card"><div class="card-body">
        <h1>Log In</h1>
        <p class="meta" style="margin-bottom:1.5rem">Landlord portal</p>
        <form id="login-form">
    ...
    app.querySelector('#login-form').onsubmit = async (e) => {
      ...
      try {
        ...
        if (res.data.user.role !== 'LAND_LORD') {
          RE.ui.toast('This portal is for landlords. Redirecting...', 'error');
  ...
  async register(app) {
    app.innerHTML = `
      <div class="auth-card card"><div class="card-body">
        <h1>Register</h1>
        <p class="meta" style="margin-bottom:1.5rem">Create a landlord account</p>
    ...
    app.querySelector('#register-form').onsubmit = async (e) => {
      ...
      fd.set('role', 'LAND_LORD');
      try {
        const res = await RE.api.auth.register(fd);
        RE.auth.login(res.data);
        RE.ui.toast('Account created! Please upload your verification document.', 'success');
